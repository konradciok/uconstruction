...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const topic = request.headers.get('x-shopify-topic') || 'collections/unknown';
    webhookEvent = topic;
...
async function handleCollectionCreate(collection: any): Promise<void> {
  try {
    console.log(`[Webhook] Creating collection: ${collection.title} (${collection.id})`);
    ...
    await prisma.collection.upsert({
      ...
      update: {
        ...
        shopifyUpdatedAt: collection.updated_at ? new Date(collection.updated_at) : null,
      },
      create: {
    ...
    console.log(`[Webhook] Successfully created collection: ${collection.title}`);
  } catch (error) {
    console.error(`[Webhook] Error creating collection ${collection.id}:`, error);
    throw error;
  }
}
...
async function handleCollectionUpdate(collection: any): Promise<void> {
  try {
    console.log(`[Webhook] Updating collection: ${collection.title} (${collection.id})`);
    ...
    if (!existingCollection) {
      console.log(`[Webhook] Collection ${collection.id} not found, creating it`);
    ...
    console.log(`[Webhook] Successfully updated collection: ${collection.title}`);
  } catch (error) {
    console.error(`[Webhook] Error updating collection ${collection.id}:`, error);
    throw error;
  }
}
...
async function handleCollectionDelete(collection: any): Promise<void> {
  try {
    ...
    console.log(`[Webhook] Successfully soft-deleted collection: ${collection.id}`);
  } catch (error) {
    // If collection doesn't exist, that's okay for delete operations
    if (error instanceof Error && error.message.includes('Record to update not found')) {
      console.log(`[Webhook] Collection ${collection.id} already deleted or doesn't exist`);
      return;
    }
    
    console.error(`[Webhook] Error deleting collection ${collection.id}:`, error);
    throw error;
  }
}